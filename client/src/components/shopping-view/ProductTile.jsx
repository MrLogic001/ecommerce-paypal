      ...
      <div onClick={() => handleGetProductDetails(product._id)}>
        ...
        <CardContent className="p-4">
             ...
             <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-[16px] text-muted-foreground">{product.price}</span>
             </div>
               <div className="flex justify-between items-center mb-2">
                <span className={`${product?.price > 0 ? 'line-through' : ''} text-lg font-semibold text-primary`}>{product?.price}</span>
               {
                product.salePrice > 0 ? <span className="text-normal font-semibold text-primary">{product.salePrice}</span> : null
               }
             </div>
        </CardContent>
